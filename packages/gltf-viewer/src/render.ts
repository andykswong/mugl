...
import { getCameraProjection, getExtras, getAccessorVertexFormat, getAccessorData, isColorVec3 } from './utils';
 ...
 */
export function renderGlTF(canvas: Canvas, device: Device, asset: GlTFAsset, options: RenderGlTFOptions = {}): void {
  const scene = (options.scene ?? asset.glTF.scene) || 0;
  const nodes = asset.glTF.nodes;
  const rootNodes = asset.glTF.scenes?.[scene]?.nodes;
  if (!nodes || !rootNodes) {
    return; // Nothing to render
  }

  const activeNodes = updateGlTF(asset, { scene });
  ...
  if (options.camera) {
    const activeCamera = asset.glTF.cameras?.[options.camera.index || 0];
    if (activeCamera) {
  ...
  for (let i = 0; i < activeNodes.length; ++i) {
    ...
    if (mesh) {
      ...
      loadModelGPUBuffer(device, node, model);
      loadMorphGPUBuffer(device, asset.glTF, node, mesh, true);
      loadJointGPUTexture(device, asset.glTF, node, true);

      for (let i = 0; i < mesh.primitives.length; ++i) {
        const primitive = mesh.primitives[i];
        if (asset.glTF.materials?.[primitive.material!]?.alphaMode === 'BLEND') {
          transparentDrawables.push({ node, mesh, primitive });
        } else {
          renderGlTFPrimitive(device, asset, node, mesh, primitive);
        }
      }
    }
  }
  ...
  for (const drawable of transparentDrawables) {
    renderGlTFPrimitive(device, asset, drawable.node, drawable.mesh, drawable.primitive);
  }
...
function renderGlTFPrimitive(
  device: Device, asset: GlTFAsset,
  node: glTF2.Node, mesh: glTF2.Mesh, primitive: glTF2.MeshPrimitive,
): void {
  let indexed = false;
  let offset = 0;
  let vertexCount = asset.glTF.accessors?.[primitive.attributes.POSITION!]?.count || 0;
  ...
  // Bind pipeline
  const pipeline = loadGPUPipeline(device, asset, primitive);
  WebGL.setRenderPipeline(device, pipeline)

  // Bind groups
  WebGL.setBindGroup(device, 0, loadMaterialBindGroup(device, asset, primitive.material));
  WebGL.setBindGroup(device, 1, loadModelBindGroup(device, asset.glTF, node, mesh));

  // Bind vertex buffers
  const buffers = getVertexBufferLayouts(asset, primitive);
  ...
  // Bind index buffer
  const indexAccessor = asset.glTF.accessors?.[primitive.indices!];
  if (indexAccessor) {
    const indexBuffer = loadGPUBuffer(device, asset, primitive.indices, BufferUsage.Index);
    if (indexBuffer) {
...
function loadGPUPipeline(device: Device, asset: GlTFAsset, primitive: glTF2.MeshPrimitive): RenderPipeline {
  ...
  const material = asset.glTF.materials?.[primitive.material!];
  ...
  const indexFormat = (asset.glTF.accessors?.[primitive.indices!]?.componentType === IndexFormat.UInt32) ?
  ...
  const pipelines: Record<string, RenderPipeline> = getExtras(asset.glTF).pipelines =
    (getExtras(asset.glTF).pipelines as Record<string, RenderPipeline>) || {};
  ...
  pipeline = getExtras(primitive).pipeline = pipelines[pipelineKey] = WebGL.createRenderPipeline(device, {
    ...
    buffers,
    bindGroups: [
      loadMaterialBindGroupLayout(device, asset.glTF),
      loadModelBindGroupLayout(device, asset.glTF),
    ],
    primitive: {
      indexFormat,
      topology,
...
function getVertexBufferLayouts(asset: GlTFAsset, primitive: glTF2.MeshPrimitive): NamedVertexBufferLayout[] {
  ...
  function getBufferLayoutDescriptor(accessor: glTF2.Accessor): NamedVertexBufferLayout {
    if (!accessor.sparse) {
      const buffer = getAccessorData(asset, accessor).buffer;
    ...
    const bufferLayout = {
      attributes: [],
      stride: asset.glTF.bufferViews?.[accessor.bufferView!]?.byteStride || 0 // TODO: will byteStride ever be undefined?
    };
  ...
  for (let shaderLocation = 0; shaderLocation < ATTRIBUTES.length; ++shaderLocation) {
    const accessor = asset.glTF.accessors?.[primitive.attributes[ATTRIBUTES[shaderLocation]]];
    if (accessor) {
  ...
  if (primitive.targets) {
    const accessors: (glTF2.Accessor | undefined)[] = new Array(3);
    ...
    for (let i = 0; i < Math.min(primitive.targets.length, maxTargets); ++i) {
      ...
      for (let j = 0; j < TARGET_ATTRIBUTES.length; ++j) {
        if ((accessors[j] = asset.glTF.accessors?.[primitive.targets[i][TARGET_ATTRIBUTES[j]]])) {
          ++attrCount;
        }
      }
...
function loadGPUBuffer(device: Device, asset: GlTFAsset, accessorId: number | undefined, usage: BufferUsage): Buffer | null {
  ...
  if (accessor.sparse || isUByteIndex) {
    ...
    if (!gpuBuffer) {
      const accessorData = getAccessorData(asset, accessor);
...
function loadGPUTexture(device: Device, asset: GlTFAsset, textureId: number, format: TextureFormat = TextureFormat.RGBA8): Texture {
  const texture = asset.glTF.textures?.[textureId];
  const img = asset.images?.[texture?.source ?? -1];

  if (!texture || !img) {
    return loadBlankGPUTexture(device, asset.glTF);
  }
...
function loadGPUSampler(device: Device, glTF: glTF2.GlTF, textureId: number): Sampler {
  const texture = glTF.textures?.[textureId];
  const sampler = glTF.samplers?.[texture?.sampler ?? -1];
  if (!sampler) {
...
function loadMaterialBindGroup(device: Device, asset: GlTFAsset, materialId: number | undefined): BindGroup {
  const material = asset.glTF.materials?.[materialId!];

  if (material && getExtras(material).bindGroup) {
    return getExtras(material).bindGroup as BindGroup;
  } else if (!material && getExtras(asset.glTF).defaultMaterialBindGroup) {
    return getExtras(asset.glTF).defaultMaterialBindGroup as BindGroup;
  }
  ...
  */
  ...
  const entries: BindGroupEntry[] = [
    ...
    { binding: 1, texture: loadBlankGPUTexture(device, asset.glTF) }, // baseColorTexture
    { binding: 2, sampler: loadDefaultGPUSampler(device, asset.glTF) }, // baseColorTexture
    { binding: 3, texture: loadBlankGPUTexture(device, asset.glTF) }, // metallicRoughnessTexture
    { binding: 4, sampler: loadDefaultGPUSampler(device, asset.glTF) }, // metallicRoughnessTexture
    { binding: 5, texture: loadBlankGPUTexture(device, asset.glTF) }, // normalTexture
    { binding: 6, sampler: loadDefaultGPUSampler(device, asset.glTF) }, // normalTexture
    { binding: 7, texture: loadBlankGPUTexture(device, asset.glTF) }, // occlusionTexture
    { binding: 8, sampler: loadDefaultGPUSampler(device, asset.glTF) }, // occlusionTexture
    { binding: 9, texture: loadBlankGPUTexture(device, asset.glTF) }, // emissiveTexture
    { binding: 10, sampler: loadDefaultGPUSampler(device, asset.glTF) }, // emissiveTexture
  ];
  ...
  if (material) {
    if (material.pbrMetallicRoughness) {
      ...
      if (pbr.baseColorTexture) {
        entries[1].texture = loadGPUTexture(device, asset, pbr.baseColorTexture.index, TextureFormat.SRGBA8);
        entries[2].sampler = loadGPUSampler(device, asset.glTF, pbr.baseColorTexture.index);
        materialData[6] = pbr.baseColorTexture.texCoord || 0;
      }
      if (pbr.metallicRoughnessTexture) {
        entries[3].texture = loadGPUTexture(device, asset, pbr.metallicRoughnessTexture.index);
        entries[4].sampler = loadGPUSampler(device, asset.glTF, pbr.metallicRoughnessTexture.index);
        materialData[7] = pbr.metallicRoughnessTexture.texCoord || 0;
      }
    }
    if (material.normalTexture) {
      entries[5].texture = loadGPUTexture(device, asset, material.normalTexture.index);
      entries[6].sampler = loadGPUSampler(device, asset.glTF, material.normalTexture.index);
      materialData[8] = material.normalTexture.scale ?? 1;
      materialData[9] = material.normalTexture.texCoord || 0;
    }
    if (material.occlusionTexture) {
      entries[7].texture = loadGPUTexture(device, asset, material.occlusionTexture.index);
      entries[8].sampler = loadGPUSampler(device, asset.glTF, material.occlusionTexture.index);
    ...
    if (material.emissiveTexture) {
      entries[9].texture = loadGPUTexture(device, asset, material.emissiveTexture.index, TextureFormat.SRGBA8);
      entries[10].sampler = loadGPUSampler(device, asset.glTF, material.emissiveTexture.index);
  ...
  const bindGroup = WebGL.createBindGroup(device, {
    layout: loadMaterialBindGroupLayout(device, asset.glTF),
    entries,
  });
  if (material) {
    getExtras(material).bindGroup = bindGroup;
  } else {
    getExtras(asset.glTF).defaultMaterialBindGroup = bindGroup;
  }

  return bindGroup;
}

function loadModelBindGroup(device: Device, glTF: glTF2.GlTF, node: glTF2.Node, mesh: glTF2.Mesh): BindGroup {
  if (getExtras(node).modelBindGroup) {
    return getExtras(node).modelBindGroup as BindGroup;
  }

  const entries: BindGroupEntry[] = [
    { binding: 0, buffer: loadModelGPUBuffer(device, node) },
...
function loadModelGPUBuffer(
  device: Device, node: glTF2.Node, data: Float32Array | null = null
): Buffer {
...
function loadMorphGPUBuffer(
  device: Device, glTF: glTF2.GlTF, node: glTF2.Node, mesh: glTF2.Mesh, update = false
): Buffer {
...
function loadModelBindGroupLayout(device: Device, glTF: glTF2.GlTF): BindGroupLayout {
...
function loadMaterialBindGroupLayout(device: Device, glTF: glTF2.GlTF): BindGroupLayout {
...
function loadJointGPUTexture(device: Device, glTF: glTF2.GlTF, node: glTF2.Node, update = false): Texture {
...
function loadBlankGPUTexture(device: Device, glTF: glTF2.GlTF): Texture {
...
function loadDefaultGPUSampler(device: Device, glTF: glTF2.GlTF): Sampler {
...
function loadJointGPUSampler(device: Device, glTF: glTF2.GlTF): Sampler {
...
function loadDefaultMorphGPUBuffer(device: Device, glTF: glTF2.GlTF): Buffer {
