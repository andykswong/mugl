...
import { glTF2 } from '@muds/gltf';

const I4 = mat4.create();

export function getDefaultCamera(glTF: glTF2.GlTF, sceneId: number, direction: Vec3, aspectRatio: number): { model: Mat4, proj: Mat4 } {
  ...
  const zFar = distance + (longestDistance * 6);
...
export function getSceneExtents(outMin: Vec3, outMax: Vec3, glTF: glTF2.GlTF, scene: glTF2.Scene): [outMin: Vec3, outMax: Vec3] {
  for (let i = 0; i < 3; ++i) {
    outMin[i] = Infinity;
    outMax[i] = -Infinity;
  }
...
function getAccessorExtents(outMin: Vec3, outMax: Vec3, accessor: glTF2.Accessor, model: Mat4) {
